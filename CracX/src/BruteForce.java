...
public class BruteForce extends Observable implements Runnable{
    ...
    private int maxPWLength; // maximale PW-Laenge
    private int minPWLength; // minimale PW-Laenge
    ...
    public void run() {
        running = true;
        // Combination-String mit Leerzeichen auffuellen
    ...
    int tryCombination(int index) {
        for (int i = 0; (i < charset.length() && running); i++) { // ueber jedes
                                                                  // Zeichen des
                                                                  // Charsets
            ...
            if (ret == 0 || ret == 1) {
            ...
            } else { // ansonsten betreffenden Index durch naechstes Zeichen aus
                     // Charset ersetzen
                String temp = combination;
                combination = combination.substring(0, index)
                        + charset.charAt(i); // Kombination aendern...
                ...
                if (index > 0) { // zunaechst die niedrigsten Stellen
                    // inkrementieren
